...
/**
 * Payment failure landing.
 *
 * Default target for /api/orders' `failureUrl`. The order row is in
 * FAILED status by the time the user lands here (or about to be once
 * the webhook fires). Adapt UI per project.
 */
interface PageProps {
...
export default async function PaymentFailurePage({ searchParams }: PageProps) {
  ...
  return (
    <main className="mx-auto max-w-md px-6 py-16 text-center">
      <h1 className="mb-3 text-2xl font-semibold">Payment not completed</h1>
      <p className="mb-6 text-gray-600">
        We couldn&apos;t complete your payment. Please try again or use a different method.
      </p>
      {o ? (
