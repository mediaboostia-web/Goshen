...
// (order-expiration was removed along with the Order/Bictorys/withdrawals
// subsystem — see PRUNING.md; recurring-expense-reminders was added for
// PRD F19.)
//
...
describe('vercel.json schema (CRON-07, D-20)', () => {
  ...
  it('declares schedules for the 6 canonical crons (post-prune + F19 reminder cron)', () => {
    if (!existsSync(VERCEL_JSON)) return;
    const cfg = JSON.parse(readFileSync(VERCEL_JSON, 'utf8')) as VercelConfig;
    ...
    expect(paths).toEqual([
      ...
      '/api/cron/recurring-expense-reminders',
